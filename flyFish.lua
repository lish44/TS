...
function rnd(n1, n2, switch)
    if switch then if switch == 1 then return rndP({n1, n2}) end end
    return {n1, n2};
end
...
function slidP(p)
    ...
    local d = math.floor((((X2 - X1) ^ 2 + (Y2 - Y1) ^ 2) ^ (1 / 2)) /
                             (jD or 20))
    local x, y = (X2 - X1) / d, (Y2 - Y1) / d
    touchDown(1, X1, Y1)
    for i = 1, d do
        touchMove(1, X1 + x * i + rnd(i, i + 5, 1),
                  Y1 + y * i + rnd(i, i + 5, 1))
        mSleep(rnd(10, 15, 1));
    end
    touchUp(1, X2, Y2)
...
function Range(x1, y1, x2, y2) return {x1, y1, x2, y2}; end

function slid(p1, p2) return {p1, p2}; end
...
function findmulcolor(tz)
    ...
    if #tz == 7 then
        -- nLog("识色")
        x, y = findMultiColorInRegionFuzzy(tz[1], tz[2], tz[3], tz[4], tz[5],
                                           tz[6], tz[7], tz[8] or {orient = 2});
    elseif #tz == 8 then
        nLog("识字")
        x, y =
            tsFindText(tz[1], tz[2], tz[3], tz[4], tz[5], tz[6], tz[7], tz[8]);
    end
    if x > 0 then
        -- nLog("找到了"..tz.name)
        return x, y
...
function DoOptFc(p, pointFound)
    ...
    elseif str == "table" then
        if #p == 3 then
            if type(p[1]) == "number" then
                CONTENTS_CLICK = "「默认点击坐标1」" .. pointFound[1] ..
                                     "+" .. p[1] .. "," .. pointFound[2] .. "+" ..
                                     p[2];
                nLog(CONTENTS_CLICK)
                ...
                local rx = rndP(p[1]);
                local ry = rndP(p[2]);
                CONTENTS_CLICK = "「默认点击坐标2」" .. pointFound[1] ..
                                     "+" .. rx .. "," .. pointFound[2] .. "+" ..
                                     ry;
                nLog(CONTENTS_CLICK)
    ...
    elseif str == "function" then
        CONTENTS_CLICK = "「执行函数」";
        nLog(CONTENTS_CLICK)
        if p() == Exit then return Exit; end
    end
end
-- 执行多个操作
function MulitOptfFc(tab, pointFound)
    ...
    for k, v in pairs(tab) do
        if DoOptFc(v, pointFound) == Exit then return Exit; end
        mSleep(100)
    end
end
...
function Engine(jobs, fcNotFound, second, time)
    ...
    while (true) do
        ...
        for k, v in pairs(jobs) do
            ...
            if findmulcolorsOne(v.FeatureExclude) > 0 then
                ...
                if (jobState == 0 or jobState == curState) and x > 0 then
                    ...
                    if v.Frequency then
                        if v.DoOpt then
                            if os.time() - v.Frequency[1] > v.Frequency[2] /
                                1000 then
        ...
        if featureNotFound then
            ...
            if timeDif > 5 then curState = 0; end
        end
        ...
        if second then if runTimes >= second then break end end

        if time then if os.time() - NntryTime > time then break end end
